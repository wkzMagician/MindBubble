...
final databaseDataVersionProvider = StreamProvider<int>((ref) async* {
  ...
  var last = -1;
  while (true) {
    final value = (await database.rawQuery('PRAGMA data_version'))
        .single
        .values
        .first as int;
    if (value != last) {
...
final bubbleRepositoryProvider = Provider<BubbleRepository>((ref) =>
    BubbleRepository(ref.watch(databaseProvider).connection,
...
class BubbleRepository {
  ...
  Future<List<Bubble>> getAll(
    ...
    final rows = await _database.query('bubbles',
  ...
  Future<void> save(Bubble bubble) async {
    await _database.insert('bubbles', bubble.toMap(),
  ...
  Future<void> delete(String id) async {
    ...
    await _database.update('bubbles', {'deleted_at': now, 'updated_at': now},
  ...
  Future<void> updateFrequency(String id, int frequency) async {
    ...
    await _database.update(
        'bubbles', {'appearance_frequency': frequency, 'updated_at': now},
        where: 'id = ?', whereArgs: [id]);
    _changed();
  }

  Future<DailySelection?> getDailySelection(String date) async {
    final rows = await _database.query('daily_selections',
    ...
    return DailySelection(
        ...
        bubbleIds: (row['bubble_ids']! as String)
            ...
            .toList(),
        generatedAt:
            DateTime.fromMillisecondsSinceEpoch(row['generated_at']! as int));
  }

  Future<void> saveDailySelection(DailySelection selection) => _database.insert(
      'daily_selections',
      {
        'date': selection.date,
